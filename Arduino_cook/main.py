...
class Game:
    def __init__(self):
        ...
        # Se a variável 'self.controler' for falsa, o jogo utilizará um arduino
    ...
    def new_game(self):
        self.new_hs = False
        if self.controler:
            self.player = Player(330, 55, 5)
        else:
            self.player = Player(330, 55)
        ...
        for item in self.itens:
                if item.hitbox[1] < self.player.hitbox[1] + self.player.hitbox[3] and item.hitbox[1] + item.hitbox[3] > self.player.hitbox[1]:
                    if item.hitbox[0] + item.hitbox[2] > self.player.hitbox[0] and item.hitbox[0] < self.player.hitbox[0] + self.player.hitbox[2]:
                        ...
                        if self.itens[self.itens.index(item)].image == self.itens[self.itens.index(item)].maca and self.player.life_cont < 5:
                            self.player.life_cont += 1
    ...
    def check_keys(self):
        ...
        elif self.keys[pg.K_d] and self.player.x + self.player.width < WIDTH - 96:
        ...
        elif self.keys[pg.K_s] and self.player.y < HEIGHT - 70:
            self.player.left = False
            self.player.right = False
            self.player.up = False
